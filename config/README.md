...
2. Each package (e.g. `fast_core`, `fast_kafka`, `fast_webrtc`, `fast_channels`) resolves config paths in this order:
...
├── README.md           ← this file
├── db/
│   └── config.json     ← DB (fast_core)
├── cache/
│   └── config.json     ← Redis cache (fast_core)
├── dynamo/
│   └── config.json     ← DynamoDB (fast_core)
├── kafka/
│   └── config.json     ← Kafka (fast_kafka)
├── webrtc/
│   └── config.json     ← WebRTC (fast_webrtc)
├── channels/
...
Configurations that still live in `fast_mvc_main/configurations/` can be moved to their respective packages (e.g. `fast_core`) using the same pattern: loader in the package, path from `FASTMVC_<X>_CONFIG_PATH` or `FASTMVC_CONFIG_BASE/<name>/config.json`. This directory remains the place to override in the main repo.
