...
Configurations that still live in `fast_mvc_main/configurations/` can be moved to their respective packages (e.g. `fast_platform`) using the same pattern: loader in the package, path from `FASTMVC_<X>_CONFIG_PATH` or `FASTMVC_CONFIG_BASE/<name>/config.json`. This directory remains the place to override in the main repo.
