...
FastX is a high-performance, developer-centric framework built on top of FastAPI. It enforces clean, vertical architecture while automating the most repetitive parts of API development through a powerful CLI.

---

## 🏗️ Architectural Excellence

FastX projects follow a "Vertical Slice" architecture by default. Instead of grouping all logic by technical layer (e.g., all controllers in one folder), it groups by **business context and operation**.
...
Scaffold a complete, production-ready authentication layer with a single command: `fastx add auth`.
...
FastX automates the setup of local environment variables:
...
- **Environment Management:** Use `fastx add env` at any time to refresh or generate local configurations.

---

## 🌐 HTTP/3 (QUIC) Edge Proxy Support

FastX’s Docker Compose setup can optionally enable an HTTP/3 (QUIC) reverse proxy using the `caddy` profile (publishes UDP `443`), in addition to the existing `nginx` TCP/TLS proxy.

---

## 📣 gRPC Transport (Health-first)

FastX can optionally start a gRPC server alongside FastAPI (enabled via `GRPC_ENABLED=true`).
...
- `protos/fastx/grpc/health/v1/health.proto`
- `protos/fastx/grpc/user/v1/user.proto`

These contracts describe the initial demo methods; gRPC stubs are generated from the proto contracts to keep the transport thin while still reusing your vertical-slice services.
...
Documentation is a first-class citizen in FastX.

- **OpenAPI:** Native FastAPI interactive Swagger and ReDoc.
- **Auto-Discovery Reference:** Run `fastx docs generate` to crawl your code and build a dedicated MkDocs API Reference using `mkdocstrings`.
...
FastX handles background processing with a clear, decoupled pattern:
...
FastX makes testing a habit, not a chore.
