...
class IWorkflowEngine(ABC):
    @abstractmethod
    async def start_order_workflow(self, order_id: str, tenant_id: str, payload: Dict[str, Any]) -> str:  # pragma: no cover - interface
        raise NotImplementedError

    @abstractmethod
    async def get_order_status(self, workflow_id: str) -> Dict[str, Any]:  # pragma: no cover - interface
        raise NotImplementedError


class TemporalWorkflowEngine(IWorkflowEngine):
    def __init__(self, address: str, namespace: str, task_queue: str) -> None:
        if _temporal_client_cls is None:  # pragma: no cover - optional
    ...
    async def _ensure_client(self) -> Any:
        if self._client is None:
            self._client = await _temporal_client_cls.connect(  # type: ignore[call-arg]
                self._address,
    ...
    async def start_order_workflow(self, order_id: str, tenant_id: str, payload: Dict[str, Any]) -> str:
        ...
        # The actual workflow type ("OrderWorkflow") is expected to be registered
        # in the worker process; here we only start it by name.
        handle = await client.start_workflow(  # type: ignore[call-arg]
            "OrderWorkflow",
    ...
    async def get_order_status(self, workflow_id: str) -> Dict[str, Any]:
        client = await self._ensure_client()
        try:
            handle = client.get_workflow_handle(workflow_id)  # type: ignore[call-arg]
            info = await handle.describe()  # type: ignore[call-arg]
            return {
...
class PrefectWorkflowEngine(IWorkflowEngine):
    def __init__(self, api_url: Optional[str], default_deployment: Optional[str]) -> None:
        if _prefect_client_cls is None:  # pragma: no cover - optional
            raise RuntimeError("prefect is not installed")
        self._client = _prefect_client_cls(api=api_url) if api_url else _prefect_client_cls()  # type: ignore[call-arg]
        self._deployment = default_deployment

    async def start_order_workflow(self, order_id: str, tenant_id: str, payload: Dict[str, Any]) -> str:
        if not self._deployment:
            raise RuntimeError("Prefect default deployment is not configured")
        flow_run = await self._client.create_flow_run_from_deployment(  # type: ignore[call-arg]
    ...
    async def get_order_status(self, workflow_id: str) -> Dict[str, Any]:
        try:
            fr = await self._client.read_flow_run(workflow_id)  # type: ignore[call-arg]
...
class DagsterWorkflowEngine(IWorkflowEngine):
    def __init__(self, grpc_endpoint: str, job_name: str) -> None:
        if _dagster_client_cls is None:  # pragma: no cover - optional
            raise RuntimeError("dagster-grpc is not installed")
        self._client = _dagster_client_cls(grpc_endpoint)  # type: ignore[call-arg]
    ...
    async def start_order_workflow(self, order_id: str, tenant_id: str, payload: Dict[str, Any]) -> str:
        ...
        def _launch_sync() -> str:
            run = self._client.submit_job_execution(  # type: ignore[call-arg]
    ...
    async def get_order_status(self, workflow_id: str) -> Dict[str, Any]:
        ...
        def _status_sync() -> Dict[str, Any]:
            run = self._client.get_run(workflow_id)  # type: ignore[call-arg]
