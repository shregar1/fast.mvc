"""
Identity provider abstractions and thin client implementations.

These are intentionally minimal and framework-agnostic. They provide
enough structure for FastMVC apps to plug in login flows without
forcing a specific authentication library.
"""
...
class IIdentityProvider:
    ...
    async def build_authorization_url(self, state: str) -> str:  # pragma: no cover - interface
        raise NotImplementedError

    async def exchange_code_for_token(self, code: str) -> Dict[str, Any]:  # pragma: no cover - interface
        raise NotImplementedError

    async def fetch_user_profile(self, access_token: str) -> IdentityUserProfile:  # pragma: no cover - interface
        raise NotImplementedError
...
class _OAuth2Provider(IIdentityProvider):
    ...
    async def exchange_code_for_token(self, code: str) -> Dict[str, Any]:
        if httpx is None:  # pragma: no cover - optional
            raise RuntimeError("httpx is required for OAuth2 flows")
...
def build_identity_providers() -> dict[str, IIdentityProvider]:
    ...
    Only enabled providers with the required minimal configuration
    are instantiated.
    """
    ...
    def _add_oauth2(
        ...
        if not dto.enabled:
            return
        auth_url = dto.auth_url or default_auth_url
        token_url = dto.token_url or default_token_url
        userinfo_url = dto.userinfo_url or default_userinfo_url
        if not (dto.client_id and dto.client_secret and auth_url and token_url and userinfo_url):
            logger.warning(
                "Identity provider %s is enabled but missing required configuration",
                key,
            )
            return
        providers[key] = _OAuth2Provider(
            ...
            redirect_uri=dto.redirect_uri or "",
            scopes=list(dto.scopes or []),
        )
    ...
    if cfg.saml.enabled:
        logger.warning(
            ...
            "placeholder. Integrate a SAML library such as python3-saml "
            "or an IdP-specific SDK for full support.",
        )
